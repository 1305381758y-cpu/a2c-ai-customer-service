...
import type { CustomerRecord, MerchantConfigRecord, Repositories } from "../repositories.js";
...
export function appConfigForMerchant(config: AppConfig, merchantConfig: MerchantConfigRecord, country?: CountryRuntimeConfig, customer?: Pick<CustomerRecord, "aiProvider" | "aiModel">): AppConfig {
  const provider = customer?.aiProvider || merchantConfig.aiProvider || config.AI_PROVIDER;
  const model = customer?.aiModel || "";
  return {
    ...config,
    ...
    MINIMAX_MODEL: provider === "minimax" && model ? model : merchantConfig.minimaxModel || config.MINIMAX_MODEL,
    MINIMAX_BASE_URL: config.MINIMAX_BASE_URL,
    DEEPSEEK_API_KEY: merchantConfig.deepseekApiKey || config.DEEPSEEK_API_KEY,
    DEEPSEEK_MODEL: provider === "deepseek" && model ? model : merchantConfig.deepseekModel || config.DEEPSEEK_MODEL,
    DEEPSEEK_BASE_URL: config.DEEPSEEK_BASE_URL,
    GOOGLE_AI_API_KEY: merchantConfig.googleAiApiKey || config.GOOGLE_AI_API_KEY,
    GOOGLE_AI_MODEL: provider === "gemini" && model ? model : merchantConfig.googleAiModel || config.GOOGLE_AI_MODEL,
...
export function appConfigForConversation(config: AppConfig, repos: Repositories, conversation: Parameters<Repositories["updateConversation"]>[0]): AppConfig {
  ...
  return appConfigForMerchant(config, merchantConfig, country, repos.getCustomer(conversation.merchantId, conversation.customerPhone));
}
