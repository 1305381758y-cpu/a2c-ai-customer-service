...
function registerMerchantOwnInviteCodeRoutes(app: FastifyInstance, deps: MerchantInviteCodeRoutesDeps): void {
  ...
  app.post<{ Params: { id: string }; Body: { codes?: string; registerUrl?: string; reusable?: boolean } }>("/api/merchant/a2c/accounts/:id/invite-codes/import", { preHandler: deps.merchantAdmins }, async (request, reply) => sendResult(reply, importInviteCodes(deps.repos, request.params.id, request.body ?? {}, scopedMerchantId(request))));
