...
export function registerMerchantCustomerRoutes(app: FastifyInstance, deps: MerchantCustomerRoutesDeps): void {
  app.get<{ Querystring: { countryId?: string; status?: string; language?: string; q?: string; startAt?: string; endAt?: string; timeZone?: string; limit?: string } }>("/api/merchant/customers", { preHandler: deps.merchantRoles }, async (request) => (
