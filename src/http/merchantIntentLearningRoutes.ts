...
export function registerMerchantIntentLearningRoutes(app: FastifyInstance, deps: MerchantIntentLearningRoutesDeps): void {
  app.get<{ Querystring: { countryId?: string; status?: string; suggestedIntent?: string; q?: string; startAt?: string; endAt?: string; timeZone?: string; limit?: string } }>("/api/merchant/intent-learning", { preHandler: deps.merchantRoles }, async (request) => (
