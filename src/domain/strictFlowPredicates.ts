...
export function reportsLinkLoadFailure(text: string): boolean {
  ...
  if (/^(还是)?(打不开|打不開|无法打开|無法打開|开不了|開不了|进不去|進不去|打不开了|打不開了)$/i.test(normalized)) return true;
  if (/^(no puedo acceder|no logro acceder|no puedo entrar|no me deja entrar|no puedo abrirlo|no puedo cargarlo|todav[ií]a no se puede cargar|todav[ií]a no puedo abrirlo|todav[ií]a no carga|sigue sin cargar|sigue sin abrir|no carga|no abre)$/i.test(normalized)) return true;
  return /(还是.*(打不开|打不開|开不了|開不了|进不去|進不去|加载不出来|載入不出來|无法加载|無法載入)|我说.*(打不开|打不開|无法打开|無法打開|无法加载|無法載入)|没有报错.*(打不开|打不開|加载不出来|无法加载|空白)|没报错.*(打不开|打不開|加载不出来|无法加载|空白)|無報錯.*(打不開|載入不出來)|链接.*(一直加载|加载不出来|无法加载|載入不出來|空白|没反应|沒有反應)|页面.*(加载不出来|无法加载|載入不出來|空白|没内容|沒有內容)|无法加载内容|載入不了內容|cannot load|won'?t load|still.*not.*open|still.*cannot.*open|page.*blank|page.*not.*load|no puedo acceder.*(enlace|link)|no logro acceder.*(enlace|link)|no puedo abrir.*(enlace|link)|no puedo (abrir|cargar)(lo|la|el)?|no se puede (abrir|cargar|acceder)|(todav[ií]a|a[uú]n).{0,20}no.{0,20}(carga|abre|abrir|acceder|puede cargar)|(enlace|link).*no.*(carga|cargar|abre|abrir|acceder)|p[aá]gina.*no.*(carga|cargar|abre)|no carga.*(p[aá]gina|enlace|link))/i.test(text);
}
