...
import { FolderPlus, Link2, Plus, Users } from "lucide-react";
...
function GroupInviteEditor({ code, teacherTgLinks, reload }: { code: InviteCode; teacherTgLinks: TeacherTgLink[]; reload: () => Promise<void> }) {
  ...
  return <article className="group-invite-row">
    ...
    <div className="teacher-binding-grid">
      <strong>绑定导师链接</strong>
      {teacherTgLinks.map((link) => <label key={link.id}><input type="checkbox" checked={selectedTeacherIds.includes(link.id)} onChange={(event) => setSelectedTeacherIds(event.target.checked ? [...selectedTeacherIds, link.id] : selectedTeacherIds.filter((id) => id !== link.id))} /><span>{link.label || link.url}<small>优先级 {link.priority} · 轮询 {link.rotationCount}</small></span></label>)}
      {!teacherTgLinks.length && <small>当前国家还没有导师链接，请先在“TG接管”中添加。</small>}
    </div>
    <div className="toolbar wrap">
      <AsyncButton busyText="保存中..." onClick={async () => { await api(`/api/merchant/a2c/group-invite-codes/${code.id}`, { method: "PATCH", body: JSON.stringify(draft) }); await api(bindingUrl, { method: "PUT", body: JSON.stringify({ teacherTgLinkIds: selectedTeacherIds }) }); await reload(); notify("success", "邀请码和导师绑定已保存"); }}>保存</AsyncButton>
      ...
      <span>{displayValue("status", code.status)}</span>
    </div>
  </article>;
}
