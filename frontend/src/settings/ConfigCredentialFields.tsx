...
const CONFIG_FIELDS: Record<ConfigCredentialGroup, string[]> = {
  a2c: ["a2cBaseUrl", "a2cAppId", "a2cAppSecret", "a2cAccountPhone"],
  ai: ["aiProvider"],
  telegram: ["telegramBotToken"],
  fallback: ["platformRegisterUrl", "tgRegisterGuideUrl"]
};

const SECRET_FIELDS = new Set(["a2cAppSecret", "minimaxApiKey", "deepseekApiKey", "googleAiApiKey", "telegramBotToken"]);
...
export function ConfigSetupSteps({ platform }: { platform: boolean }) {
  return <div className="setup-strip">
    <div><span>1</span><strong>填写接入信息</strong><small>{platform ? "智能供应商 / TG" : "A2C / TG"}</small></div>
