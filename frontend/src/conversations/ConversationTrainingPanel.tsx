...
export function TrainingLoopPanel({
  ...
  return <section className="training-loop-panel">
    ...
    {activeTab === "assistant" && <>
      ...
      <details className="assistant-card assistant-collapsible">
        <summary className="assistant-card-title"><BookOpen size={17}/><div><h3>匹配知识</h3><p>{conversation.countryName ? `${countryLabel(conversation.countryName)} · ${languageName(conversation.language)}` : "当前客户上下文"}</p></div><span className="status-pill ok">{firstKnowledge ? "已匹配" : "待补充"}</span></summary>
      ...
      <details className="assistant-card assistant-collapsible script-guidance">
        <summary className="assistant-card-title"><Workflow size={17}/><div><h3>脚本引导</h3><p>{scriptFlow?.flow.name || "系统流程"} · 当前步骤：{label(flowStep)}</p></div></summary>
      ...
      <details className="assistant-card assistant-collapsible">
        <summary className="assistant-card-title"><FileText size={17}/><div><h3>样本推荐</h3><p>相似场景优秀回复</p></div><span className="status-pill neutral">{review.items.length || trainingSamples.length ? "有推荐" : "暂无"}</span></summary>
      ...
      <details className="assistant-card assistant-collapsible">
        <summary className="assistant-card-title"><Lightbulb size={17}/><div><h3>训练提升</h3><p>当前对话可沉淀为训练内容</p></div><span className="status-pill neutral">{review.review ? `${review.review.score}分` : "未复盘"}</span></summary>
