...
function MerchantConversations({ handoffs = false, timeMode, canApplyTraining, canDelete }: { handoffs?: boolean; timeMode: TimeDisplayMode; canApplyTraining: boolean; canDelete: boolean }) {
  ...
  const defaultFilters: Filters = { status: handoffs ? "human_handoff" : "", handoffStatus: handoffs ? "pending" : "", language: "", ...todayDateTimeRange("Asia/Shanghai") };
  ...
  return <div className={`conversation-workspace ${customerCollapsed ? "customers-collapsed" : ""}`}>
    ...
    <ConversationCustomerList
      ...
      renderFilterBar={() => <FilterBar filters={filters} setFilters={setFilters} fields={handoffs ? ["language", "startAt", "endAt"] : ["status", "handoffStatus", "language", "startAt", "endAt"]} selects={{ status: ["", "active", "human_handoff"], handoffStatus: ["", "pending", "processing", "done"] }} resetValues={defaultFilters} onApply={reloadRows} />}
    />
    <section className="chat-pane">{selected ? <ConversationDetail canApplyTraining={canApplyTraining} canDelete={canDelete} conversation={selected} refresh={async () => { await reloadRows(); await reloadUnread(); }} onClose={() => setSelected(null)} onDeleted={async () => { setSelected(null); await reloadRows(); await reloadUnread(); }} /> : selectedAccount && draftCustomer ? <ProactiveConversationDetail account={selectedAccount} target={draftCustomer} onClose={() => setDraftCustomer(null)} onCreated={async (conversation) => { setSelected(conversation); setDraftCustomer(null); setNewCustomer({ customerPhone: "", nickname: "" }); await reloadRows(); await reloadUnread(); }} /> : <div className="empty-chat export-empty-state"><h3>选择客户开始对话</h3><p>左侧选择客服账号，中间选择客户；也可以使用顶部工具条一键导出全部线上对话用于复盘、训练或交给同事分析。</p></div>}</section>
  </div>;
}
