...
export function MerchantsPage() {
  ...
  return <div className="split merchant-admin-layout"><CountryPresetDatalist />
    ...
    <section className="detail-panel">{selected ? <div><div className="detail-title-row"><h3>商户详情</h3><button type="button" className="ghost icon-only panel-close-button" title="关闭详情" aria-label="关闭详情" onClick={() => { setSelected(null); setSelectedCountry(null); setSelectedUser(null); }}>×</button></div><MerchantDetailPanel merchant={selected} detailError={detailError} selectedCountry={selectedCountry} users={users} selectedUser={selectedUser} onMerchantChange={setSelected} onCountryChange={setSelectedCountry} onUserChange={setSelectedUser} onReloadMerchants={reloadMerchants} onReloadDetail={reloadMerchantDetail} /></div> : <div className="empty-state">选择商户后可修改名称和状态。新增商户时可以同时创建国家和商户端登录账号。</div>}</section>
  </div>;
}
