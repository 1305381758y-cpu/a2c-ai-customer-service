...
type ChatMessage = { id: number; direction: string; content: string; msgType: string; language: string; intent: string; createdAt: string; rawPayload?: { originalContent?: string; translatedContent?: string; targetLanguage?: string; translationStatus?: "translated" | "skipped" | "failed"; translationError?: string; operatorTranslatedContent?: string; operatorTranslationTargetLanguage?: string; operatorTranslationStatus?: "translated" | "skipped" | "failed"; operatorTranslationError?: string; manual?: boolean } };
...
type VectorStatus = { status: string; sourceType: string; count: number };
type VectorHit = { id: number; sourceType: string; sourceId: string; title: string; content: string; language: string; intent: string; stage: string; score: number };
type RetrievalGroup = { messageId: number; createdAt: string; hits: VectorHit[] };
...
function VectorIndexStatusStrip() {
  const [rows, setRows] = useState<VectorStatus[]>([]);
  const reload = async () => setRows((await api<{ rows: VectorStatus[] }>("/api/merchant/vector-index/status")).rows);
  useEffect(() => { reload().catch(() => null); }, []);
  const total = rows.reduce((sum, item) => sum + Number(item.count || 0), 0);
  if (!total) return <div className="vector-strip"><span>语义索引：暂无记录</span><small>上传素材、样本或收到对话后会自动排队。</small></div>;
  return <div className="vector-strip">{["embedded", "pending", "failed"].map((status) => {
    const count = rows.filter((item) => item.status === status).reduce((sum, item) => sum + item.count, 0);
    return <span key={status} className={status === "failed" ? "danger-text" : ""}>{label(status)} {count}</span>;
  })}<AsyncButton className="ghost" busyText="重建中..." onClick={async () => { const result = await api<{ queued: number; embedded: number; failed: number }>("/api/merchant/vector-index/rebuild", { method: "POST", body: JSON.stringify({ embedNow: true }) }); notify("success", "语义索引已重建", `排队 ${result.queued} 条，完成 ${result.embedded} 条，失败 ${result.failed} 条`); await reload(); }}><RefreshCw size={15}/>重建</AsyncButton></div>;
}

function VectorIndexPage() {
  const [rows, setRows] = useRows<VectorStatus>("/api/merchant/vector-index/status");
  const total = rows.reduce((sum, item) => sum + Number(item.count || 0), 0);
  const reload = async () => setRows((await api<{ rows: VectorStatus[] }>("/api/merchant/vector-index/status")).rows);
  const cards = ["embedded", "pending", "failed"].map((status) => ({
    status,
    count: rows.filter((item) => item.status === status).reduce((sum, item) => sum + item.count, 0)
  }));
  return <div className="single-column work-split"><section className="work-panel vector-page"><div className="vector-hero"><div><h3>语义记忆索引</h3><p>客户对话、优秀样本和知识库会被向量化，用于相似问题检索和回复引用。图片只使用说明或 OCR 文字，不会把图片链接当手机号学习。</p></div><AsyncButton busyText="重建中..." onClick={async () => { const result = await api<{ queued: number; embedded: number; failed: number }>("/api/merchant/vector-index/rebuild", { method: "POST", body: JSON.stringify({ embedNow: true }) }); notify("success", "重建完成", `排队 ${result.queued} 条，已向量化 ${result.embedded} 条，失败 ${result.failed} 条`); await reload(); }}><RefreshCw size={16}/>重建全部索引</AsyncButton></div><div className="grid metrics vector-metrics">{cards.map((item) => <section key={item.status} className="metric-card"><div className="metric-top"><span>{label(item.status)}</span><i><Workflow size={18}/></i></div><strong>{item.count}</strong><small>{vectorStatusHint(item.status)}</small></section>)}</div><Table rows={rows} columns={["sourceType", "status", "count"]} /><div className="notice">当前共 {total} 条语义索引。索引只在当前商户和当前国家内检索，不会跨商户引用。</div></section></div>;
}

function vectorStatusHint(status: string) {
  if (status === "embedded") return "已可参与语义检索";
  if (status === "pending") return "等待生成向量";
  if (status === "failed") return "可重建后重试";
  return "索引状态";
}
...
function Customers({ platform = false }: { platform?: boolean }) {
  ...
  const deleteSelected = async () => {
    if (!selected) return;
    if (!window.confirm(`确认彻底删除客户 ${selected.customerKey}？该客户的所有会话、聊天记录、记忆、接管记录和语义索引都会一起删除。`)) return;
    const url = platform
...
function ConversationDetail({ platform = false, conversation, refresh, onDeleted }: { platform?: boolean; conversation: Conversation; refresh: () => void; onDeleted?: () => Promise<void> | void }) {
  ...
  const [retrievals, setRetrievals] = useState<RetrievalGroup[]>([]);
