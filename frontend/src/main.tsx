...
function Config({ platform }: { platform: boolean }) {
  ...
  const [countryDraft, setCountryDraft] = useState({ code: "br", name: "Brazil", defaultLanguage: "pt-BR", platformRegisterUrl: "", tgRegisterGuideUrl: "", requirePlatformAccount: "true", requirePhone: "true", requireTelegram: "true", requireWhatsApp: "false" });
...
function Customers({ platform = false }: { platform?: boolean }) {
  ...
  return <div className="split"><section><FilterBar filters={filters} setFilters={setFilters} fields={platform ? ["merchantId", "countryId", "status", "language", "limit"] : ["countryId", "status", "language", "limit"]} selects={{ countryId: ["", ...countries.map((country) => country.id)], status: ["", "active", "human_handoff"] }} onApply={async () => setRows(await loadRows(rowsUrl))} /><Table rows={rows} columns={columns} onRow={setSelected} /></section><section>{selected ? <div><h3>{selected.customerKey}</h3><p>{selected.countryName} · {selected.nickname || "无昵称"} · {selected.status} · {selected.language}</p><div className="form-grid"><label>首次接收账号<input readOnly value={selected.firstA2CAccountPhone || ""} /></label><label>最近接收账号<input readOnly value={selected.lastA2CAccountPhone || ""} /></label><label>手机号<input readOnly value={selected.extractedPhone || ""} /></label><label>Telegram<input readOnly value={selected.extractedTelegram || ""} /></label><label>WhatsApp<input readOnly value={selected.extractedWhatsApp || ""} /></label><label>会话数<input readOnly value={String(selected.conversationCount || 0)} /></label><label>最近会话ID<input readOnly value={selected.lastConversationId || ""} /></label></div><p>客户档案由 A2C Webhook 自动创建和更新；后台账号仍在“后台账号”里单独管理。</p></div> : <p>客户第一次发消息后会自动出现在这里，不需要手动创建。</p>}</section></div>;
}
...
function ChatBubble({ message }: { message: ChatMessage }) {
  ...
  const sendIssue = isOutbound && payload.a2cSendStatus === "failed" ? `A2C发送失败：${payload.a2cSendError || "未知错误"}` : "";
  return <article className={`chat-bubble ${message.direction}`}><div className="bubble-meta"><span>{isOutbound ? "客服" : "客户"}</span><time>{formatTime(message.createdAt)}</time></div>{original ? <div className="translation-block"><strong>{isOutbound ? "客服原文" : "客户原文"}</strong><p>{original}</p>{canShowTranslation ? <><strong>{isOutbound ? "发送译文" : "中文译文"}{payload.targetLanguage ? ` · ${payload.targetLanguage}` : ""}</strong><p>{translated}</p></> : <div className="translation-warning">{translationIssue}</div>}</div> : <p>{message.content}</p>}{sendIssue && <div className="translation-warning">{sendIssue}</div>}<small>{message.intent} · {message.language}</small></article>;
}
...
function label(key: string) {
  return ({
    ...
    googleAiApiKey: "Google AI Studio Key", googleAiModel: "Gemini模型", openaiApiKey: "OpenAI Key", openaiModel: "OpenAI模型", telegramBotToken: "TG机器人", telegramHandoffChatId: "TG群ID",
